...
function contact (){
  const contactInfo = [];
  const displayName = graduates.forEach(graduates => contactInfo.push(`${graduates.first_name} ${graduates.email}`));
  return contactInfo;
}
...
/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
  ...
  // const unisWithUni = [];
  const unisWithUni = (graduates.filter(universities => universities.university === "Uni") );
...
function getDisplayNames (){
  const displayNames = [];
  const displayName = zooAnimals.forEach(zooAnimals => displayNames.push(`Name: ${zooAnimals.animal_name}, Scientific: ${zooAnimals.scientific_name}.`));
  return displayNames;
}

console.log(getDisplayNames());
...
*/
  ...
  // const totalPop = zooAnimals.reduce((total, current) => {
